...
%              obj.v               = 1;
            global f1 f2;
%             disp('reached R1')
            global x_E1 y_E1  x_R1 y_R1 x_E2 y_E2 x_R2 y_R2 x_R3 y_R3 x_R4 y_R4 R2_engage R3_engage R4_engage
            global persuit_E1 persuit_E2 R1_engage  R1E1 R1E2 R2E1 R2E2 R3E1 R3E2 R4E1 R4E2      
        %% EXECUTE Selects and executes the current controller.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Assign Robots to Evader 1 %%%%%%%%%%%%%%%%%%%%%%%
                    dist_E1R=[2 2 2];
                if R2_engage==0
                    dist_E1R(1)=sqrt(((x_E1-x_R2)^2)+((y_E1-y_R2)^2));
                end
                
                if R3_engage==0
                    dist_E1R(2)=sqrt(((x_E1-x_R3)^2)+((y_E1-y_R3)^2));
                end
                
                if R4_engage==0
                    dist_E1R(3)=sqrt(((x_E1-x_R4)^2)+((y_E1-y_R4)^2));
                end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                    
                if persuit_E2 == 0 && R1_engage==0
                    dist_E2=sqrt(((x_E2-x)^2)+((y_E2-y)^2));

                    if dist_E2<=0.3
                        persuit_E2=1;
                        
%%%%%%%%%%%%%%%%%%%%%%% Assign Robots for Evader 2 %%%%%%%%%%%%%%%%%%%%%%%%%%                         
                        dist_E2R=[2 2 2];
                if R2_engage==0
                    dist_E2R(1)=sqrt(((x_E2-x_R2)^2)+((y_E2-y_R2)^2));
                end
                
                if R3_engage==0
                    dist_E2R(2)=sqrt(((x_E2-x_R3)^2)+((y_E2-y_R3)^2));
                end
                
                if R4_engage==0
                    dist_E2R(3)=sqrt(((x_E2-x_R4)^2)+((y_E2-y_R4)^2));
                end
                X=min(dist_E2R);
                I=find(dist_E2R==X,1); 
...
%             fprintf('Estimated pose (x,y,theta): (%0.3g,%0.3g,%0.3g)\n', x_new, y_new, theta_new);
            ...
            function I=assign_robots_E1()
                global x_E1 y_E1 x_R2 y_R2 x_R3 y_R3 x_R4 y_R4 R2_engage R3_engage R4_engage
            ...
            function I=assign_robots_E2()
                global x_E2 y_E2 x_R2 y_R2 x_R3 y_R3 x_R4 y_R4 R2_engage R3_engage R4_engage 
